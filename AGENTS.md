...
## CLI — `cli/`, `app/device/`, `docs/mcp.md`
...
- Commands: `login` (device authorization), `whoami`, `logout`, `add <title>`, `list [--filter]`, `done <id>`, `mcp --stdio` — all but `login`/`logout`/`mcp` need a stored token and call the REST API above over `fetch`, importing `@ai-tutor/todo-api-schema` to parse the responses.
...
- `cli/src/mcp.ts` (`runMcpServer`, run by `mcp --stdio`) is on `@modelcontextprotocol/server`/`stdio`, the package the current MCP docs point to (not the older `@modelcontextprotocol/sdk`); its three tools' `inputSchema`s are the shared `@ai-tutor/todo-api-schema` zod objects directly (`done`'s just needs `todoSchema.shape.id`, wrapped), so the CLI, the REST API, and the MCP tools can't drift on shape.
- The server starts unconditionally regardless of login state; each tool's handler calls the shared `requireToken()` (in `cli/src/api.ts`, also used by every CLI command) and turns a thrown `NotAuthenticated`/`ApiError` into a tool result with `isError: true`, never a thrown exception — the one and only thing written to stdout is the JSON-RPC wire, so all logging in `cli/src/mcp.ts` goes to `console.error`.
- `cli/tests/test-server.ts` is the one test harness both `cli.integration.test.ts` and `mcp.integration.test.ts` call into: a real `next dev` on a free port (its `distDirSuffix` param must differ per test file so their `next dev` locks don't collide), a fresh temp database with one seeded user, and a `testUtils()`-backed `betterAuth` instance on that same database for minting that user's sessions without a password or a browser.
- `cli/tests/mcp.integration.test.ts` drives the *built* `mcp --stdio` with the real `@modelcontextprotocol/client` SDK (`StdioClientTransport` + `Client`) — pre-seeds a valid login by calling `saveToken()` from `cli/src/config.ts` directly, then exercises `tools/list` and `add`/`list`/`done`, plus a second client pointed at an empty config dir to check the not-logged-in error text.
- `docs/mcp.md` covers registering the server with Claude Code (`claude mcp add`, absolute path to `cli/dist/index.js`, project vs. user scope) and confirming it connected (`claude mcp list`, `/mcp`) — verified against a real `claude mcp add`/`list`/`get`/`remove` round trip while writing it, not just the upstream docs.
...
- Running either `cli/tests/*.integration.test.ts` spawns `next dev` against its own `.next-cli-test`/`.next-mcp-test`, which — like `.next-e2e` — may re-append its own `tsconfig.json` include entries; harmless, and the same "just commit it" situation as the `nextjs-agent-rules` block at the top of this file.
